...
export default function BudgetPage() {
  ...
  return (
    <div className="grid-lg" style={{ "--accent": BudgetItem?.color }}>
      <GoBack />
      <h1 className="h2">
        <span className="accent">{BudgetItem?.budname}</span> Overview
      </h1>
      <div className="flex-lg">
        {BudgetItem ? <BudgetDetails budget={BudgetItem} showDel={true} /> : ""}

        <ExpensesAdder
          budgets={BudgetItem ? JSON.stringify([BudgetItem]) : null}
        />
        {ExpenseItem && JSON.parse(fetchData("new-expense")).length > 0 && (
          <div className="grid-lg">
            <h2>
              Recent<span className="accent"> {BudgetItem?.budname} </span>
              Expenses
            </h2>
